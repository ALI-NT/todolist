...
class TaskManagerApp:
    def __init__(self, root):
        ...
        # Task storage (list of tuples: (task_text, is_done, timestamp))
        self.tasks = []
    ...
    def add_task(self):
        ...
        # Add task to list with timestamp (False means not done)
        self.tasks.append((task, False, timestamp))
        self.update_listbox()
    ...
    def done_task(self):
        try:
            selected_index = self.task_listbox.curselection()[0]
            task_text, is_done, timestamp = self.tasks[selected_index]
            
            # Toggle done status (keep timestamp)
            self.tasks[selected_index] = (task_text, not is_done, timestamp)
            self.update_listbox()
        except IndexError:
            pass
    ...
    def update_listbox(self):
        ...
        for i, (task, is_done, timestamp) in enumerate(self.tasks, 1):
            if is_done:
                display_text = f"{i}. ✓ {task} - [{timestamp}] (Done)"
                self.task_listbox.insert(tk.END, display_text)
                self.task_listbox.itemconfig(tk.END, fg="gray")
            else:
                display_text = f"{i}. {task} - [{timestamp}]"
                self.task_listbox.insert(tk.END, display_text)
